...
def preprocess_data(proj_name):
    ...
    for rel in cur_all_rel:
        ...
        filtered_df = all_df[(~all_df['is_comment']) & (~all_df['is_blank'])]
        filtered_df['line_number'] = (
                filtered_df.groupby('filename').cumcount() + 1
        )
        filtered_df.to_csv(save_dir + rel + ".csv", index=False)
