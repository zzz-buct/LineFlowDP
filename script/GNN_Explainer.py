...
class GNNExplainer(torch.nn.Module):
    ...
    def explain_graph(self, data, **kwargs):
        ...
        try:
            ...
            edge_type = data["node", "edge", "node"].edge_type if "edge_type" in data["node", "edge", "node"] else None

        batch = torch.zeros(x.shape[0], dtype=int, device=x.device)

        with torch.no_grad():
            if edge_type is not None:
                out = self.model(x, edge_index, edge_type, batch)
            else:
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            if edge_type is not None:
                out = self.model(h, edge_index, edge_type, batch)
            else:
                out = self.model(h, edge_index, batch)
            log_logits = self.__to_log_prob__(out)
